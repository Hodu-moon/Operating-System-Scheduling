...
class HRRN extends Scheduler{
    ...
    public void schedule(){
        ...
        for(int i = 0; i < readyQueue.size(); i++){
            if ( high_Response_ratio < readyQueue.get(i).getResponeRatioTime(currentTime)){
                high_Response_ratio = readyQueue.get(i).getResponeRatioTime(currentTime);
                index = i;
            }
        }
...
class SRT extends Scheduler{
    ...
    public void schedule(){
        super.schedule();
        int min_remaining_time = 999999999;
        int min_index = 0;
        for(int i = 0; i < readyQueue.size(); i++){
            if ( min_remaining_time > readyQueue.get(i).getRemainingTime()){
                min_remaining_time = readyQueue.get(i).getRemainingTime();
                min_index = i;
            }
        }
        if(readyQueue.size() >= 1)
            currentProcess = readyQueue.get(min_index);
    }
...
class SPN extends Scheduler{
    ...
    public void schedule(){
        super.schedule();
        int min_process_serviceTime = 999999999;
        int min_index = 0;
        for(int i = 0; i < readyQueue.size(); i++){
            if ( min_process_serviceTime > readyQueue.get(i).getServiceTime()){
                min_process_serviceTime = readyQueue.get(i).getServiceTime();
                min_index = i;
            }
        }
        if(readyQueue.size() >= 1)
